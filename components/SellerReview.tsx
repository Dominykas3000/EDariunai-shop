...
const SellerReviews = (props: any) => {
  ...
  let sellerId = props.props;

  useEffect(() => {
    async function fetchReview() {
      const response = await fetch(`/api/seller-review?userId=${session?.user?.id}&sellerId=${sellerId}`);
      if (response.ok) {
          ...
          <div className="w-full flex flex-col  justify-start gap-3  items-start mb-2">
            <div className="w-full flex flex-col">
              ...
              <select
                ...
                className="bg-gray-50 mt-2 border border-gray-300 text-gray-900 text-base rounded-lg focus:ring-gray-900 focus:border-gray-900 block w-full p-2.5  dark:border-gray-500 dark:placeholder-gray-400"
              >
